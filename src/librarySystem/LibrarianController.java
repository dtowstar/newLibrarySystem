...
public class LibrarianController {
	...
	public void addMemberButtonClick(ActionEvent event) {
		...
		b1.addActionListener(ActionEvent -> {
			...
			int opt = JOptionPane.showConfirmDialog(ADF, "確定是否新增會員？", "確定", JOptionPane.YES_NO_OPTION, mType);
					...
					if (getcheckR.isEmpty()) {
						...
						if (getcheckRMRID.isEmpty()) {
							try {
								...
								JOptionPane.showMessageDialog(ADF, "會員已存在", "結束", JOptionPane.INFORMATION_MESSAGE);
							} catch (Exception e) {
								JOptionPane.showMessageDialog(ADF, "沒有輸入會員", "Error", JOptionPane.ERROR_MESSAGE);
							}
						} else {
							JOptionPane.showMessageDialog(ADF, "身分證字號已新增", "Error", JOptionPane.ERROR_MESSAGE);
						}
					} else {
						JOptionPane.showMessageDialog(ADF, "MemberID已新增", "Error", JOptionPane.ERROR_MESSAGE);
					}
				} else if (opt == JOptionPane.NO_OPTION) {
					// borrowFrame.setDefaultCloseOperation(WindowConstants.EXIT_ON_CLOSE);
					JOptionPane.showMessageDialog(ADF, "已新增", "結束", JOptionPane.INFORMATION_MESSAGE);
				}
			} else {
				JOptionPane.showMessageDialog(ADF, "memberID 的開頭字母必須要是M", "Error", JOptionPane.ERROR_MESSAGE);
			}
		});
	}
	...
	public void addEbookButtonClick(ActionEvent event) {
		...
		JButton getPath = new JButton("已新增仵嚙�");
		JButton save = new JButton("嚙編嚙磕ebook");
		...
		jp1.add(new JLabel("bookTitle嚙瘦"));
		jp1.add(bookTitle);
		jp1.add(new JLabel("author嚙瘦"));
		jp1.add(author);
		jp1.add(new JLabel("publisher嚙瘦"));
		jp1.add(publisher);
		jp1.add(new JLabel("publicationdate嚙瘦"));
		jp1.add(publicationdate);
		jp1.add(new JLabel("summary 嚙瘦"));
		jp1.add(summary);
		jp1.add(getPath);
		jp1.add(label);
		jp1.add(save);
		...
		save.addActionListener(Actionevent -> {
			int mType = JOptionPane.INFORMATION_MESSAGE;
			int opt = JOptionPane.showConfirmDialog(jf, "嚙緲嚙線嚙踝蕭嚙緯嚙編嚙磕Ebook嚙豌？", "確定", JOptionPane.YES_NO_OPTION, mType);
			if (opt == JOptionPane.YES_OPTION) {
				...
				JOptionPane.showMessageDialog(jf, "會員已存在ebook嚙踝蕭id嚙瞌" + InitialGUI.getmaxID(), "結束",
						JOptionPane.INFORMATION_MESSAGE);
			} else if (opt == JOptionPane.NO_OPTION) {
				JOptionPane.showMessageDialog(jf, "已新增", "結束", JOptionPane.INFORMATION_MESSAGE);
			}
		});
		getPath.addActionListener(Actionevent -> {
			int result = chooser.showDialog(jf, "嚙罷嚙課歹蕭嚙踝蕭嚙�");
	...
	public void addBookButtonClick(ActionEvent event) {
		...
		p1.add(new JLabel("bookTitle嚙瘦"));
		p1.add(bookTitle);
		p1.add(new JLabel("author嚙瘦"));
		p1.add(author);
		p1.add(new JLabel("publisher嚙瘦"));
		p1.add(publisher);
		p1.add(new JLabel("publicationDate嚙瘦"));
		p1.add(publicationDate);
		p1.add(new JLabel("summary嚙瘦"));
		p1.add(summary);
		p1.add(new JLabel("state嚙瘦"));
		p1.add(p2);
		p1.add(new JLabel("price嚙瘦"));
		p1.add(price);
		...
		creatPaperBook.addActionListener(ActionEvent -> {
			int mType = JOptionPane.INFORMATION_MESSAGE;
			int opt = JOptionPane.showConfirmDialog(APBF, "嚙緲嚙線嚙踝蕭嚙緯嚙編嚙磕嚙諸嗎？", "確定", JOptionPane.YES_NO_OPTION, mType);
			if (opt == JOptionPane.YES_OPTION) {
				...
				JOptionPane.showMessageDialog(APBF, "會員已存在PaperBook ID:" + InitialGUI.getmaxID(), "結束",
						JOptionPane.INFORMATION_MESSAGE);
			} else if (opt == JOptionPane.NO_OPTION) {
				JOptionPane.showMessageDialog(APBF, "已新增", "結束", JOptionPane.INFORMATION_MESSAGE);
			}
		});
	}
	...
	public void editMemberButtonClick(ActionEvent event) {
		...
		JFrame editMemberFrame = new JFrame("嚙論改蕭member已新增");
		JPanel p = new JPanel();
		editMemberFrame.setDefaultLookAndFeelDecorated(true);
		String input = JOptionPane.showInputDialog("嚙請選蕭JmemberID");
		if (input != null || input != "") {
			try {
			...
			if (haveMember.getmemberID() == null || haveMember.getmemberID() == "") {
				...
				b1.addActionListener(ActionEvent -> {
					...
					int mType = JOptionPane.INFORMATION_MESSAGE;
					int opt = JOptionPane.showConfirmDialog(editMemberFrame, "嚙緲嚙線嚙踝蕭嚙緯嚙論改蕭|嚙踝蕭嚙豌？", "確定",
					...
					if (opt == JOptionPane.YES_OPTION) {
						...
						JOptionPane.showMessageDialog(editMemberFrame, "會員已存在", "結束", JOptionPane.INFORMATION_MESSAGE);
					} else if (opt == JOptionPane.NO_OPTION) {
						// borrowFrame.setDefaultCloseOperation(WindowConstants.EXIT_ON_CLOSE);
						JOptionPane.showMessageDialog(editMemberFrame, "已新增", "結束", JOptionPane.INFORMATION_MESSAGE);
					}
				});
	...
	public void editEbookButtonClick(ActionEvent event) {
		...
		JFileChooser chooser = new JFileChooser(".");
		JButton getPath = new JButton("已新增仵嚙�");
		JButton save = new JButton("嚙論改蕭ebook");
		JLabel accessory = new JLabel();
		JLabel label = new JLabel();
		JLabel labelID = new JLabel();
		chooser.setAcceptAllFileFilterUsed(false);
		EDBF.setDefaultLookAndFeelDecorated(true);
		String input = JOptionPane.showInputDialog("嚙請選蕭JEbookID");
		if (input != null || input != "") {
			try {
				getebook = LibraryDBMgr.searchData(input, "searchebook");
			} catch (Exception e) {
				System.out.println("嚙盤嚙糊嚙篌嚙瞎");
			}
			...
			getPath.addActionListener(Actionevent -> {
				int result = chooser.showDialog(EDBF, "嚙罷嚙課歹蕭嚙踝蕭嚙�");
				if (result == JFileChooser.APPROVE_OPTION) {
					String name = f.getName().toLowerCase();
					if (name.endsWith(".txt")) {
						String namep = chooser.getSelectedFile().getPath();
			...
			save.addActionListener(Actionevent -> {
				Member sendMember = new Member();
				int mType = JOptionPane.INFORMATION_MESSAGE;
				int opt = JOptionPane.showConfirmDialog(EDBF, "嚙緲嚙線嚙踝蕭嚙緯嚙論改蕭ebook嚙豌？", "確定", JOptionPane.DEFAULT_OPTION,
						mType);
				if (opt == JOptionPane.YES_OPTION) {
					...
					JOptionPane.showMessageDialog(EDBF, "嚙踝蕭嚙穀嚙論改蕭", "結束", JOptionPane.INFORMATION_MESSAGE);
				} else if (opt == JOptionPane.NO_OPTION) {
					// borrowFrame.setDefaultCloseOperation(WindowConstants.EXIT_ON_CLOSE);
					JOptionPane.showMessageDialog(EDBF, "已新增", "結束", JOptionPane.INFORMATION_MESSAGE);
				}
			});
		}
	}
	...
	public void editBookButtonClick(ActionEvent event) {
		...
		String input = JOptionPane.showInputDialog("嚙請選蕭JPaperbookID");
		if (input != null || input != "") {
			try {
			...
			if (getPaperBook.isEmpty()) {
				...
				editPaperBook.addActionListener(ActionEvent -> {
					...
					int mType = JOptionPane.INFORMATION_MESSAGE;
					int opt = JOptionPane.showConfirmDialog(EPBF, "嚙緲嚙線嚙踝蕭嚙緯嚙論改蕭捅隉H", "確定", JOptionPane.YES_NO_OPTION,
							mType);
					if (opt == JOptionPane.YES_OPTION) {
						...
						JOptionPane.showMessageDialog(EPBF, "會員已存在PaperBook ID:" + InitialGUI.getmaxID(), "結束",
								JOptionPane.INFORMATION_MESSAGE);
					} else if (opt == JOptionPane.NO_OPTION) {
						JOptionPane.showMessageDialog(EPBF, "已新增", "結束", JOptionPane.INFORMATION_MESSAGE);
					}
				});
			}
		}
	...
	public void deleteMemberButtonClick(ActionEvent event) {
		JFrame deleteMFrame = new JFrame("嚙磋嚙踝蕭member已新增");
		JButton b1 = new JButton("嚙磋嚙踝蕭");
		JTextField ID = new JTextField(15);
		JPanel p = new JPanel();
		p.add(new JLabel("嚙踝蕭J嚙緯嚙磋嚙踝蕭member嚙踝蕭ID"));
		p.add(ID);
		p.add(b1);
		deleteMFrame.add(p);
		deleteMFrame.pack();
		...
		b1.addActionListener(ActionEvent -> {
			...
			int mType = JOptionPane.INFORMATION_MESSAGE;
			int opt = JOptionPane.showConfirmDialog(deleteMFrame, "嚙緲嚙線嚙踝蕭嚙緯嚙磋嚙踝蕭嚙罵嚙踝蕭嚙豌？", "確定", JOptionPane.YES_NO_OPTION,
					mType);
			if (opt == JOptionPane.YES_OPTION) {
				...
				if (checkMember.getnumberOfBorrowBook() != 0 || checkMember.getnumberOfNoticeBook() != 0
					...
					JOptionPane.showMessageDialog(deleteMFrame, "嚙踝蕭嚙穀嚙磋嚙踝蕭嚙罵嚙踝蕭", "結束", JOptionPane.INFORMATION_MESSAGE);
				}
			} else if (opt == JOptionPane.NO_OPTION) {
				// borrowFrame.setDefaultCloseOperation(WindowConstants.EXIT_ON_CLOSE);
				JOptionPane.showMessageDialog(deleteMFrame, "已新增", "結束", JOptionPane.INFORMATION_MESSAGE);
			}
	...
	public void deleteEbookButtonClick(ActionEvent event) {
		JFrame deleteMFrame = new JFrame("嚙磋嚙踝蕭ebook已新增");
		JButton b1 = new JButton("嚙磋嚙踝蕭");
		JTextField ID = new JTextField(15);
		JPanel p = new JPanel();
		p.add(new JLabel("嚙踝蕭J嚙緯嚙磋嚙踝蕭ebook嚙踝蕭ID"));
		p.add(ID);
		p.add(b1);
		deleteMFrame.add(p);
		deleteMFrame.pack();
		...
		b1.addActionListener(ActionEvent -> {
			int mType = JOptionPane.INFORMATION_MESSAGE;
			int opt = JOptionPane.showConfirmDialog(deleteMFrame, "嚙緲嚙線嚙踝蕭嚙緯嚙磋嚙踝蕭Ebook嚙豌？", "確定", JOptionPane.YES_NO_OPTION,
					mType);
			if (opt == JOptionPane.YES_OPTION) {
				ArrayList<Ebook> getEbook = new ArrayList<Ebook>();
				...
				if (checkEbook != null) {
					...
					JOptionPane.showMessageDialog(deleteMFrame, "嚙踝蕭嚙穀嚙磋嚙踝蕭ebook", "結束", JOptionPane.INFORMATION_MESSAGE);
				} else {
	...
	public void borrowBookButtonClick(ActionEvent event) {
		JFrame borrowFrame = new JFrame("嚙褕書歹蕭嚙踝蕭");
		JButton b1 = new JButton("嚙褕殷蕭");
		JTextField ID = new JTextField(15);
		JPanel p = new JPanel();
		p.add(new JLabel("嚙踝蕭J嚙諸迎蕭ID"));
		p.add(ID);
		p.add(b1);
		borrowFrame.add(p);
		borrowFrame.pack();
		borrowFrame.setDefaultLookAndFeelDecorated(true);
		String input = JOptionPane.showInputDialog("嚙請選蕭JborrowerID");
		if (input == "" || input == null) {
				...
				try {
					...
					if (checkPaperBook == null) {
						...
						if (checkPaperBook.getstate().equals("available")) {
							System.out.println("嚙箠嚙皚嚙褕書訊嚙踝蕭");
							if (checkMember.getright() == true) {
								System.out.println("嚙箠嚙皚嚙確嚙緹RIGHT");
								int mType = JOptionPane.INFORMATION_MESSAGE;
								int opt = JOptionPane.showConfirmDialog(borrowFrame, "嚙緲嚙線嚙踝蕭嚙緯嚙褕書嗎？", "確定",
										JOptionPane.DEFAULT_OPTION, mType);
								if (opt == JOptionPane.YES_OPTION) {
									Date now = new Date();
									...
									JOptionPane.showMessageDialog(borrowFrame, "嚙踝蕭嚙穀嚙褕殷蕭", "結束",
											JOptionPane.INFORMATION_MESSAGE);
								} else if (opt == JOptionPane.NO_OPTION) {
									// borrowFrame.setDefaultCloseOperation(WindowConstants.EXIT_ON_CLOSE);
									borrowFrame.dispose();
	...
	public void returnBookButtonClick(ActionEvent event) {
		...
		JFrame returnBookFrame = new JFrame("嚙誶書歹蕭嚙踝蕭");
		JButton b1 = new JButton("嚙誶殷蕭");
		JTextField ID = new JTextField(15);
		JPanel p = new JPanel();
		p.add(new JLabel("嚙踝蕭J嚙緯嚙誶書迎蕭ID"));
		p.add(ID);
		p.add(b1);
		...
		b1.addActionListener(ActionEvent -> {
			...
			if (setPaperBook.getstate().equals("borrowed")) {
				...
				JOptionPane.showMessageDialog(returnBookFrame, "嚙踝蕭嚙穀嚙誶殷蕭", "結束", JOptionPane.INFORMATION_MESSAGE);
	...
	public void updatePaperBookStateButtonClick(ActionEvent event) {
		...
		JFrame updatePaperBookStateFrame = new JFrame("嚙踝蕭嚙豌書迎蕭嚙璀已新增");
		JButton b1 = new JButton("嚙踝蕭悛嚙踝蕭A");
		JPanel p = new JPanel();
		ButtonGroup bg = new ButtonGroup();
		JPanel p2 = new JPanel();
		Border line = BorderFactory.createLineBorder(Color.black);
		...
		if (input != null || input != "") {
			...
			if (getPaperBook.isEmpty()) {
				...
				havePaperBook = getPaperBook;
				setPaperBook = havePaperBook.get(0);
				p.add(new JLabel("嚙諸迎蕭ID嚙瞌" + setPaperBook.getbookID()));
				p.add(new JLabel("嚙諸迎蕭嚙踝蕭嚙璀嚙瞌" + setPaperBook.getstate()));
				...
				b1.addActionListener(ActionEvent -> {
					...
					JOptionPane.showMessageDialog(updatePaperBookStateFrame, "嚙踝蕭嚙穀:" + InitialGUI.getmaxID(), "結束",
							JOptionPane.INFORMATION_MESSAGE);
				});
			}
		} else {
